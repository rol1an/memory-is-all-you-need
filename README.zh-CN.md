...
![星图——每条记忆一颗发光体，轨道来自真实读取次数](docs/hero.png)
...
**1. 它到底记了什么？**
翻记忆要开编辑器一个个点文件。现在是一张力导向星图 + 全文抽屉：搜索、筛选、四种着色（类型 / 读取热度 / 社区聚类），一眼看全几百条记忆的形状。

**2. 记忆之间是什么关系？**
`[[wikilink]]` 埋在正文里根本看不见。现在是可见的边、Louvain 自动社区、孤儿节点和悬空引用一键透镜。
...
![读取热度模式——热记忆像余烬在烧，冷的多数退成暗点](docs/heat-mode.png)
